...
TVT package installation and training can be run using: `tvt/run.sh`. This will use
...
Note that the default installation uses Tensorflow without GPU support. Replace
`tensorflow` with `tensorflow-gpu` in `tvt/requirements.txt` to use tensorflow
with GPU.
...
Further details about the tasks are provided in the README of the Pycolab directory,
and users can also play the tasks themselves from the command line.
...
Further details about specific tasks are given in the README of the DeepMind Lab directory.
...
#### Agent configuration:
`with_memory`: default is True. Whether or not agent has external memory. If set to
...
`num_episodes` Number of episodes to train for. None means run indefinitely.<br>

#### Pycolab-specific flags:
`pycolab_game`: Which game to run. One of 'key_to_door' or
'active_visual_match'. See pycolab/README for a description.<br>
...
`pycolab_final_reward`: The reward was obtained at the last phase.<br>
...
<br>
Key values logged:
`reward`: The total rewards the agent acquired in an episode. <br>
...
this paper focuses on the reward in the last phase of each task, which is
...
best way to track whether TVT is working is by monitoring the `last phase reward`
as this is the critical performance we are interested in - the agent with TVT
and the control agents performed well in the apple collecting phase, which
contributes most of the episodic rewards, but not in the last phase.

### Key-to-door
Across 10 replicas, we found that the TVT agents achieved a score of 10,
...
For 10 replicas without TVT and with the same hyperparameters, we observe consistent
low performance.<br>
# ![No_TVT_ktd](images/ktd_notvt.png)
For 10 replicas without TVT and with gamma equal to 1, performance of the RMA
agent without TVT is improved, but it is unstable and never consistently exceeds
6.<br>
...
phase.<br>
# ![TVT_vm](images/avm_tvt.png)For 10 replicas without TVT and with the same hyperparameters, performance is better than chance level but not at the maximum level, indicating that it is unable to actively seek information in the explore phase and instead must rely on randomly encountering the information.<br>
# ![No_TVT_vm](images/avm_notvt.png)
For 10 replicas without TVT and with gamma equal to 1, the performance of the RMA
agent without TVT is considerably worse, suggesting that the behavior learned from later phases does not
